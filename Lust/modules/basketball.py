...
async def roll_dart(client: Client, message: t.Message):
    ...
    if value.dice.value == 6:  # Winning only if the dice rolls a 6
        ...
        await message.reply_text(f"[🏀](https://files.catbox.moe/xeiyir.jpg) You're lucky!\nYou won {bastek_amount} Exlix")
        await add_xp(user_id, 4)
    else:
        await deduct(user_id, bastek_amount)
        await message.reply_text(f"[🍷](https://files.catbox.moe/3v05m3.jpg) Better luck next time!\nYou lost {bastek_amount} Exlix")
