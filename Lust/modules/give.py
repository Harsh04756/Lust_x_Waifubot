...
async def give_character_command(client, message):
    if not message.reply_to_message:
        await message.reply_text("Reply to a user to give character.")
        return
    try:
        ...
        await message.reply_text("Provide character ID.")
    except ValueError as e:
        await message.reply_text(str(e))
    except Exception as e:
        print(e)
        await message.reply_text("Error while processing command.")
...
async def add_all_characters_for_user(user_id):
    ...
    # _id hatao MongoDB ka — warna inline query mein clash hoga
    new_chars = [clean_char(c) for c in all_chars if c['id'] not in existing]
    if not new_chars:
        return f"No new characters for {user_id}"
    await user_collection.update_one(
        {'id': user_id},
        {'$push': {'characters': {'$each': new_chars}}}
    )
    return f"✅ Added {len(new_chars)} characters to {user_id}"


@app.on_message(filters.command(["ad"]) & sudo_filter)
async def add_characters_command(client, message):
    if not message.reply_to_message:
        await message.reply_text("Reply to user to add characters.")
        return
...
async def remove_character_command(client, message):
    try:
        ...
        if len(args) == 3:
            ...
            await message.reply_text("Usage: /blank <user_id> <char_id> or reply with /blank <char_id>")
            return
        res = await kill_character(receiver_id, character_id)
        await message.reply_text(res)
    except Exception as e:
        print(e)
        await message.reply_text("Error while removing character.")
